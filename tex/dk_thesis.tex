...
%\iff \mathcal{L}_T \le \mathcal{L}_T^{\text{min}} + 2\sqrt{T \log N}
\end{align*}
...
Blum and Mansour\cite{blum2007external} also give us a way to generate an algorithm with vanishing swap regret given an algorithm with vanishing external regret.  
talk about\cite{mohri2014conditional}

\section{Swap Regret Minimization in Signaling Games

\todo{here is where i will put a technical treatment of the algorithm and an analysis of its performance on the game as well as simulation results}
