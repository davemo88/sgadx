...
\item $T_{i}$ is the type of player $i$.
\item $\tau_{i}:\Omega \to T_{i}$ is a function that assigns types to players
\item $A_{i}$ is the set of actions for player $i$.
\item $C_{i} \subseteq A_{i} \times T_{i}$ defines the actions available to player $i$ when player $i$ is of type $T_{i}$. 
\item $u_i:\Omega \times A \to \mathbb{R}$ is the payoff function for player $i$.
\item $p_{i}$ is the probability distribution over $\Omega$ for player $i$.
\end{itemize}
